...
describe("loadConfig", () => {
  it("fetches /headless/config?shop and returns the payload", async () => {
    ...
    const fetchImpl = vi.fn(async (url) => {
      expect(url).toBe("https://hiko.link/headless/config?shop=s.myshopify.com");
      return new Response(JSON.stringify(payload), { status: 200 });
    });
    const cfg = await loadConfig({ configServer: "https://hiko.link", shop: "s.myshopify.com", fetchImpl });
    expect(cfg.providers).toEqual(["google"]);
  });

  it("throws on non-OK", async () => {
    const fetchImpl = vi.fn(async () => new Response("{}", { status: 500 }));
    await expect(loadConfig({ configServer: "https://hiko.link", shop: "x", fetchImpl })).rejects.toThrow("config_unavailable");
  });
});
